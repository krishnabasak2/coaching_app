...
@section('content')
    <div class="col-lg-12">
        <div class="breadcrumb-main">
            <h4 class="text-capitalize breadcrumb-title">All Coaching</h4>
            <div class="breadcrumb-action justify-content-center flex-wrap">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ url('admin') }}"><i class="uil uil-estate"></i>Home</a>
                        </li>
                        <li class="breadcrumb-item active" aria-current="page">All Coaching</li>
                    </ol>
                </nav>
            </div>
        </div>
                ...
                <div class="user-group radius-xl media-ui media-ui--early pt-30 pb-25">
                    <div class="border-bottom px-30">
                        <div class="media user-group-media d-flex justify-content-between">
                            <div class="media-body d-flex align-items-center flex-wrap text-capitalize my-sm-0 my-n2">
                                <a href="application-ui.html">
                                    <h6 class="mt-0  fw-500 user-group media-ui__title bg-transparent">{{ $item['name'] }}
                                    </h6>
                                </a>
                                ...
                                @elseif($item['status'] == 'suspend')
                                    <span
                                        class="my-sm-0 my-2 media-badge text-uppercase color-white bg-warning">{{ $item['status'] }}</span>
                                @endif
                            </div>
                            <div class="mt-n15">
                                    ...
                                    <div class="dropdown-menu">
                                        ...
                                        @if ($item['status'] == 'active')
                                            <a class="dropdown-item" href="#"
                                                onclick="coachingStatus({{ $item['id'] }}, 'block')">Block</a>
                                        @elseif($item['status'] == 'block')
                                            <a class="dropdown-item" href="#"
                                                onclick="coachingStatus({{ $item['id'] }}, 'active')">Active</a>
                                        @endif

                                        @if ($item['deleted_at'] == null)
                                            <a class="dropdown-item" href="#"
                                                onclick="coachingStatus({{ $item['id'] }}, 'trash')">Trash</a>
                                        @else
                                            <a class="dropdown-item" href="#"
                                                onclick="coachingStatus({{ $item['id'] }}, 'restore')">Restore</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="user-group-people mt-15 text-capitalize">
                            <p><strong>{{ $item['hade_name'] . ' - ' . $item['phone'] . ' - ' . $item['email'] }}
                                    ||</strong>
                                {{ $item['address'] }}
                            </p>
...
@push('script')
    <script>
        const coachingStatus = (id, status) => {
            swal({
                    title: "Are you sure?",
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        let url = base_url + '/admin/coaching/status/' + id + '/' + status;
